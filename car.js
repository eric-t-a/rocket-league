...
export default class Car {
    ...
    initialize() {
        ...
        this.physicsWorld.addContactMaterial(bodyGroundContactMaterial)
    }

    loadModels() {

        const boxGeometry = new THREE.BoxGeometry(this.chassisDimension.x, this.chassisDimension.y, 4);
        const boxMaterial = new THREE.MeshNormalMaterial();
        this.chassis = new THREE.Mesh(boxGeometry, boxMaterial);
        this.chassis.castShadow = true;
        this.chassis.receiveShadow = true;
        this.world.scene.add(this.chassis);
        ...
        for(let i = 0 ; i < 4 ; i++) {
            ...
            cylinderMesh.geometry.rotateY(Math.PI/2);
            this.wheels.push(cylinderMesh)

            this.world.scene.add(cylinderMesh);
        }
    }
    ...
    setChassis() {
        ...
        this.chassisBody = new CANNON.Body({mass: this.mass, material: new CANNON.Material({friction: 0})});
        this.chassisBody.addShape(chassisShape);
        this.chassisBody.position.y = 5;
        ...
        this.car.addToWorld(this.physicsWorld);
    }
    ...
    animate() {
        if(this.car.wheelInfos && this.chassis.position && this.wheels[0].position){
            this.chassis.position.set(
                ...
                this.car.chassisBody.position.y + 0.5,
        ...
        this.world.camera.lookAt( this.chassis.position );
    }
}
